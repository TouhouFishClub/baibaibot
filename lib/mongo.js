...
saveTxt = function(ask,answer,name,groupName,callback,from){
  MongoClient.connect(mongourl, function(err, db) {
    ...
    cl_txt.findOne(query, function(err, data) {
      if(data){
        if(data.lock){
          if(from==357474405){
            save(ask,answer,callback,cl_txt,name,groupName,db,from);
          }else{
            callback('记住 "'+ask+'" 了哇');
          }
        }else{
...
function save(ask,answer,callback,cl_txt,name,groupName,db,from){
  if(ask.length>0){
    if(answer==""){
