function calAttr(content,qq,callback){
  if(content.startsWith("直击")){
    ...
    var each = Math.round(((nextrate*1.25+(100-nextrate))-(rate*1.25+(100-rate)))/(rate*1.25+(100-rate))/(next-left)*1000000)/100;
    callback('直击率:'+rate+"%"+"\n直击伤害:125%\n下一个阈值:"+next+"\n每点收益:"+each);
    ...
    var each = Math.round(((nextrate*nextcx/100+(100-nextrate))-(rate*cx/100+(100-rate)))/(rate*cx/100+(100-rate))/(next-left)*1000000)/100;
    callback('暴击率:'+rate+"%"+"\n暴击伤害:"+cx+"%\n下一个阈值:"+next+"\n每点收益:"+each);
    ...
    var each = Math.round((nextrate-rate)/rate*1000000)/1000
    callback('信念收益:'+rate+"%"+"\n下一个阈值:"+next+"\n每点收益:"+each);
  }else{
