...
async function generateProactiveReply(groupId, port) {
  try {
    ...
    const recentContent = recentMessages.slice(-5)
      ...
      .replace(/[?？!！。，,.;；:：]/g, ' ')
      .trim()
    const relevantKnowledge = await knowledge.getRelevantKnowledgePrompt(recentContent, 5)
...
async function generateMentionReply(userMessage, groupId, port, userName = '用户') {
  ...
  try {
    ...
    // 查询相关知识库内容
    const relevantKnowledge = await knowledge.getRelevantKnowledgePrompt(searchText, 8)
