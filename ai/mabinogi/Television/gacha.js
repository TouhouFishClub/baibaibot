...
const scheduledMissingSyncGacha = async () => {
  try {
    console.log(`[${new Date().toISOString()}] 开始执行定时抽蛋缺失数据同步任务`)
    
    // 确保连接正常
    await checkLink()
    
    // 同步猫服抽蛋缺失数据
    console.log('定时任务: 开始检查猫服抽蛋缺失数据')
    const ylxResult = await syncMissingIdsToMongoDB('mabi_draw_reward_records')
    console.log(`定时任务: 猫服抽蛋缺失数据同步完成 - ${ylxResult.success ? '成功' : '失败'}`)
    if (ylxResult.success) {
      if (ylxResult.count > 0) {
        console.log(`定时任务: 猫服抽蛋缺失数据同步统计 - 总缺失:${ylxResult.totalMissing}, 本次同步:${ylxResult.count}`)
      } else {
        console.log(`定时任务: 猫服抽蛋无缺失数据`)
      }
    } else {
      console.error(`定时任务: 猫服抽蛋缺失数据同步失败 - ${ylxResult.message}`)
    }
    
    // 同步亚特抽蛋缺失数据
    console.log('定时任务: 开始检查亚特抽蛋缺失数据')
    const yateResult = await syncMissingIdsToMongoDB('mabi_draw_reward_records_yate')
    console.log(`定时任务: 亚特抽蛋缺失数据同步完成 - ${yateResult.success ? '成功' : '失败'}`)
    if (yateResult.success) {
      if (yateResult.count > 0) {
        console.log(`定时任务: 亚特抽蛋缺失数据同步统计 - 总缺失:${yateResult.totalMissing}, 本次同步:${yateResult.count}`)
      } else {
        console.log(`定时任务: 亚特抽蛋无缺失数据`)
      }
    } else {
      console.error(`定时任务: 亚特抽蛋缺失数据同步失败 - ${yateResult.message}`)
    }
    
    console.log(`[${new Date().toISOString()}] 定时抽蛋缺失数据同步任务完成`)
    
  } catch (error) {
    console.error(`[${new Date().toISOString()}] 定时抽蛋缺失数据同步任务执行失败:`, error.message)
  }
}
...
const startScheduledSyncGacha = () => {
  ...
  // 延迟到下一个整点，然后每小时执行一次
  setTimeout(() => {
    scheduledMissingSyncGacha() // 立即执行一次
    setInterval(scheduledMissingSyncGacha, 60 * 60 * 1000) // 然后每小时执行
  }, timeToNextHour)
  
  console.log(`抽蛋数据定时缺失同步任务已启动 - 下次执行时间: ${nextHour.toLocaleString('zh-CN')}`)
}
