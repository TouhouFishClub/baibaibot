...
const render = async (data, option) => {
  const width = option.columns.reduce((p, { key, format }) => p + Math.max(...data.map(x => (format ? format(x[key]) : x[key]).split('').reduce((sp, char) => sp + (/^[\u4E00-\u9FA5]$/.test(char) ? 24: 12), 0))), 0) + 50 * option.columns.length
  console.log(`-----\n\n\n\n${width}\n\n\n\n-----`)
