...
const createEchoStone = (group, callback, refine = false, rare = 2, care = false) => {
	...
	while (list.length < 30 && count < 30000) {
      ...
      if(care && target.drop) {
		...
		count ++
    let level = list.length - 1
    let ins = infos[level - 1].increase
    let up = popTmp > 0 ? (0 - popTmp) : list[level]
    popTmp = 0
		levelArr.push({
      l: level,
      min: ins[0],
  ...
  console.log(levelArr.map(x => `${["简单","普通","困难"][x.rare]} (${x.min}-${x.max})\t${x.up}\tlevel: ${x.l-1}`).join('\n'))
