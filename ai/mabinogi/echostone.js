...
const createEchoStone = (group, callback, refine = false, rare = 2, care = false) => {
    ...
    let ins = level > 28 ? [0] : (infos[level - 2].increase || [0])
    ...
    popTmp = 0
      ...
      avg: avgData.reduce((p, e) => p + e, 0),
      avgD: avgTmp.reduce((p, e) => p + e, 0),
      up,
      source: list.concat([]),
      rare
    })
	}
  console.log(levelArr.map(x => `${["简单","普通","困难"][x.rare]} (${x.min}-${x.max})\t${x.up}(up: ${x.avgD} avg: ${x.avg})\tlevel: ${x.l}`).join('\n'))
