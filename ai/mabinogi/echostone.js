...
const createEchoStone = (group, callback, refine = false, rare = 2, care = false) => {
	...
	while (list.length < 30 && count < 30000) {
      ...
      if(care && target.drop) {
		...
		count ++
    let level = list.length, ins = level > 28 ? [0] : infos[level - 1].increase, up = list[level - 1] - (level - 1 > 0 ? list[level - 2] : 0)
		levelArr.push({
      l: level,
      min: ins[0],
  ...
  console.log(levelArr.map(x => `${["简单","普通","困难"][x.rare]} (${x.min}-${x.max})\t${up}\tlevel: ${x.l}`).join('\n'))
