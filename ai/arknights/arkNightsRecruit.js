...
module.exports = function(qq, content, callback){
  ...
  if(!con) {
    callback(`===> 明日方舟工具箱 <===\n公招查询 【ark + tag】:\n查询出现此tag组合的3星以上干员，tag组用空格隔开，tag不能大于5个，可使用截图查询（截图tag区域）\n干员查询 【arks + 干员名称】:\n查询干员信息，干员名称可模糊搜索\n经验计算 【arke + 参数】:\n可通过arke 星级 [当前精英化等级-]当前等级 [目标精英化等级-]目标等级 [持有金币数量] [基础作战记录数量, 初级作战记录数量, 中级作战记录数量, 高级作战记录数量]查询，如arke 6 24 2-31 1000 2,3,6,100 ,其中，带中括号的参数非必填，若忽略精英化等级默认精0，等级超过当前精英化等级默认当前精英化最高级（如6星2-99默认为2-80）\n家具查询 【arkd + 家具名称】:\n可查询一套家具的舒适度\n限定作战 【arkc + 关卡名称】: \n可随机一定的关卡限制条件`)
    return
  }
  ...
  if(n > -1){
    content.substr(n).split(',').forEach(p => {
      ...
      if(sp[0] == 'url'){
        ...
        baiduocr(sp[1], d => {
          console.log('----')
          arkNight(qq, checkTags(d.replace(/干员/g, '\n').split('\n').filter(x => x.trim() != '').map(t => simMap(t))).join(' '), callback)
        })
      }
    })
  } else {
